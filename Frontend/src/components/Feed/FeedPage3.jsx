...
const FeedPage3 = ({ onNavigateBack }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            window.uiAlert?.('Failed to create employee of the month');
        }
    };

    const handleDeleteTeamMember = async (teamMemberId) => {
        if (await window.uiConfirm?.('Are you sure you want to remove this team member?')) {
            try {
                await employeeOfMonthService.deleteTeamMember(teamMemberId);
                fetchEmployeeOfMonth();
            } catch (error) {
                console.error('Error deleting team member:', error);
                window.uiAlert?.('Failed to delete team member');
            }
        }
    };

    const handleDeleteEmployeeOfMonth = async () => {
        if (await window.uiConfirm?.('Are you sure you want to delete the Employee of the Month? This will remove the entire entry and team.')) {
            try {
                ...
                window.uiAlert?.('Failed to delete employee of the month');
            }
        }
    };
