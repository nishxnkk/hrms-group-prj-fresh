...
const SignUp = () => {
  ...
  return (
    <div className="min-h-screen w-screen flex items-center justify-center font-[Poppins] px-4 md:px-12 lg:px-24 py-12 bg-[var(--rn-bg)]">
      <div className="w-full max-w-5xl bg-white rounded-lg shadow-xl overflow-hidden min-h-[75vh] border border-[var(--rn-border)]">
        <div className="flex flex-col md:flex-row">
          <div className="w-full md:w-2/3 p-6 sm:p-8 md:p-16 relative">
            ...
            <form ref={formRef} onInvalidCapture={handleInvalidCapture} id="createAccountForm" className="space-y-4 relative z-10" onSubmit={handleSubmit}>
              <div>
                <label htmlFor="name" className="block text-sm font-semibold mb-1 text-black">Name</label>
                <input ref={fieldRefs.name} id="name" name="name" type="text" required minLength={3} value={name} onChange={(e) => setName(e.target.value)} className="w-full max-w-[400px] border border-gray-300 rounded-md p-3 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base" placeholder="Your name" />
              ...
              <div>
                ...
                <input ref={fieldRefs.phone} id="phone" name="phone" type="tel" pattern="[0-9+\-()\s]{7,20}" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full max-w-[400px] border border-gray-300 rounded-md p-3 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base" placeholder="Optional" />
              ...
              <div>
                ...
                <input ref={fieldRefs.email} id="email" name="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full max-w-[400px] border border-gray-300 rounded-md p-3 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base" placeholder="you@example.com" />
                {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
              </div>

              <div>
                <label htmlFor="designation" className="block text-sm font-semibold mb-1 text-black">Designation</label>
                <select ref={fieldRefs.designation} id="designation" name="designation" required value={designation} onChange={(e) => setDesignation(e.target.value)} className="w-full max-w-[400px] border border-gray-300 rounded-md p-3 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base">
              ...
              <div className="flex items-center space-x-2">
                <input ref={fieldRefs.robot} id="robot" type="checkbox" className="h-4 w-4 accent-green-500" />
                <label htmlFor="robot" className="text-sm text-black font-medium cursor-pointer">i am not a robot</label>
              </div>

              <button type="submit" className="w-full max-w-[400px] bg-[var(--rn-primary)] text-white py-2 rounded-md font-medium hover:bg-[var(--rn-primary-hover)] transition-all text-sm sm:text-base">create account</button>
